...
//   - Build mode must match the linked engine: define USINGZ and link
//     libClipper2Z for the Z API; otherwise omit it and link libClipper2.
...
// endpoints: when the intersection point coincides with one of them it
// keeps that endpoint's z, otherwise it gets the DefaultZ, which is our
// sentinel Z_INTERSECTION. The engine's SetZ pre-seeds pt.z with this same
// policy before invoking the callback, so on that path re-deriving is a no-op.
// The callback cannot simply trust the pre-seed, though: DoSplitOp invokes
// the callback directly on a point whose z GetLineIntersectPt zeroed, never
// consulting DefaultZ — a callback leaving pt.z untouched would emit an invented
// vertex with z = 0, indistinguishable from a tagged input vertex.
// Registration is required either way: with no callback SetZ returns early and
// even normal intersections get z = 0.
...
// The Z-aware counterpart to populatenode
